...
for i, site in enumerate(sites):
    ...
    pivot_table['diffNextNumericSide'] = pivot_table[1] - pivot_table[
        0]  # Assuming 0 represents ipsi and 1 represents contra
    ...
    grouped_mouse['mirroredEvidence'] = np.where(grouped_mouse['nextContraSensoryEvidence'] > 0.5,
                                                 1 - grouped_mouse['nextContraSensoryEvidence'],
                                                 grouped_mouse['nextContraSensoryEvidence'])

    # Compute the "uncertainty" by reflecting around the midpoint and round to 3 decimal places
    grouped_mouse['uncertainty'] = grouped_mouse['nextContraSensoryEvidence'].apply(
        lambda x: 1 - np.around(np.abs(.5 - x), decimals=2))
    ...
    pivot_table = grouped_mouse.pivot_table(index=['mouse', 'uncertainty', 'DAresponseSize'], columns='numericSide',
                                            values='nextNumericSide').reset_index()
    pivot_table['diffNextNumericSide'] = pivot_table[1] - pivot_table[
        0]  # Assuming 0 represents ipsi and 1 represents contra
    ...
    agg_diff_grouped = agg_diff.groupby(['mouse', 'uncertainty', 'DAresponseSize'])[
        'diffNextNumericSide'].mean().reset_index()
    ...
    sns.lineplot(data=agg_diff_grouped, x='uncertainty', y='diffNextNumericSide', hue='DAresponseSize', ci=68,
                 palette=colors[site])
...
for i, site in enumerate(sites):
    ...
    print(
        '{}: DA size {}, uncertainty {}, interaction {})'.format(site, site_p_vals[0], site_p_vals[1], site_p_vals[2]))
