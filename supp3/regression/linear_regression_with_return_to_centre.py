...
for index, experiment in experiments_to_process.iterrows():
    ...
    var_exp = run_regression_return_to_centre_one_mouse_one_session(mouse, date, reg_type='_return_to_centre_300frames_long_turns')
    var_exps.append(var_exp)

    gc.collect()
experiments_to_process['var exp'] = var_exps
saving_folder = 'T:\\photometry_2AC\\processed_data\\'
var_exp_filename = os.path.join(saving_folder,'_'.join(mouse_ids) + '_var_exp_with_return_to_centre_300frames_long_turns.p') # '_var_exp_with_return_to_centre.p'
